            ...
            <div class="row mt-2">
                <div class="col-lg-6 col-sm-12 thanhtoan_left">
                    ...
                    <form action="" method="post" class="form__thanhtoan">
                        <div class="fullname d-lg-flex justify-content-between">
                            <div class="fullname_name ">
                                <p>Tên <span>*</span></p>
                                <input type="text" name="ten" class="ten " value="<?=$tenkhachhang?>" disabled >
                            </div>
                        ...
                        <div class="form_group mt-2">
                            ...
                            <input type="text" value="<?=$diaChi?>" name="diachi" required id="diachi">
                        </div>

                        <div class="fullname d-lg-flex w-100 justify-content-between">
                            <div class="form_group sdt ">
                                <p>Số điện thoại <span>*</span></p>
                                <input type="text" value=0<?=$sdt?> required>
                            </div>
...
checkbox.addEventListener('change', function() {
    if (checkbox.checked && diachi.value!="") {
        thanhtoan.disabled = false;
    } else {
        thanhtoan.disabled = true;
    }
});
const radioButtons = document.querySelectorAll('input[type="radio"][name="check"]');

// Lặp qua từng radio button để thêm sự kiện change
radioButtons.forEach(function(radioButton) {
    radioButton.addEventListener('change', function() {
        // Kiểm tra nếu radio button có giá trị là "tratien" thì ẩn phần tử có id là "qrcode", ngược lại thì hiển thị
        const qrcodeElement = document.getElementById('qrcode');
        if (this.value === 'tratien') {
            qrcodeElement.style.display = 'none'; // Ẩn phần tử khi giá trị là "tratien"
        } else {
            qrcodeElement.style.display = 'block'; // Hiển thị phần tử khi giá trị không phải "tratien"
        }
    });
});
