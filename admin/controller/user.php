...
if(isset($act)){
    switch($act){
     ...
     case 'trangchu':
        ...
        $get_kh = khachhang();
        $get_dh = donhang();
        $donhang_cu= donhang_cu();
        $donhang_moi= donhang_moi();
        $donhang_kh= donhang_kh();
        $_SESSION['taikhoan'];
 ...
 case 'chatchitiet':
    $showTatCa=showtatca();
    if(isset($id_kh)){
      $list_tinnhan=showtinnhan($id_kh);
    if(isset($submit)){
     themtinnhan_admin($id_kh,$tinnhan);
     header('Location:?mod=user&act=chatchitiet&id_kh='.$id_kh.'');
     $list_tinnhan=showtinnhan($id_kh);
    }  
    }
    
    include_once "./view/header.php";
    include_once "./view/chatchitiet.php";
    break;
    // khach hang
    case 'khachhang':
        $khachhang_moi=laytatcathongtin(1);
    ...
    case 'donhangchitiet':
        ...
        print_r($tatcaDh['trangthai']);
           ...
           if(isset($chuanbi)){
            ...
            header("Location:?mod=user&act=donhangchitiet&id_dh=$id_dh");
           }
            else if(isset($danggiao)){
                chinhsuaTrangThai(2,$id_dh);
                header("Location:?mod=user&act=donhangchitiet&id_dh=$id_dh");
           }
           else if(isset($khongnhan)){
            chinhsuaTrangThai(5,$id_dh);
            re_sl($id_dh);
            header("Location:?mod=user&act=donhangchitiet&id_dh=$id_dh");
        }
            else if(isset($hoantat)){
            chinhsuaTrangThai(4,$id_dh);
            header("Location:?mod=user&act=donhangchitiet&id_dh=$id_dh");
           }
